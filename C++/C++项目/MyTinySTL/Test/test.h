...
void UnitTest::Run()
{
  for (auto it : testcases_)
  {
    ...
    CurrentTestCase = testcase;// 单元测试类中的公有成员 当前测试实例
    testcase->nTestResult = 1; // testcase中的实例测试结果
    testcase->nFailed = 0;
    testcase->nPassed = 0;
    std::cout << green << "============================================\n";
    std::cout << green << " Run TestCase:" << testcase->testcase_name << "\n";
    testcase->Run(); // 类algorithm_test::copy_test_TEST 继承于TestCase // 检查数组copy过来的是否一致 (两种copy方式比较)
    if (testcase->nFailed == 0)  // 当存在copy错误时，打印控制台的颜色变为红色
      std::cout << green;
    else
      std::cout << red;
...
#define TESTCASE_NAME(testcase_name) \
    testcase_name##_TEST // ##连接作用 等于testcase_name_TEST

// 使用宏定义掩盖复杂的测试样例封装过程，把 TEXT 中的测试案例放到单元测试中
// 预处理，定义80个test类，并将测试项保存在nuitTest类保护成员的std::vector<TestCase*> testcases_;
...
#define TEST(testcase_name) \
  MYTINYSTL_TEST_(testcase_name) // 建立80个test实例

// 运行所有测试案例
#define RUN_ALL_TESTS() \
  mystl::test::UnitTest::GetInstance()->Run()//执行run函数
