...
Main.prototype = {
  init() { // 填充数据
    ...
    this.bproto = this.board.__proto__;// 对象的原型属性
  ...
  addRandomData() { //填充数据
  // 当剩余一个空格时，保持与相邻空格一致内容，确保不会阵亡
    if (this.board.usefulCell().length==1)
    {
      var cell1 = this.board.usefulCell();
      // 
      var pos_x = cell1[0].x;// 取值0,1,2,3
      var pos_y = cell1[0].y;
      var cell = this.board.selectCell(); 
      if (pos_x + 1 < 4)
      {
        var value = this.board.grid[pos_x+1][pos_y];
        cell.val = value;
      }
      else if (pos_y + 1 < 4)
      {
        var value = this.board.grid[pos_x][pos_y+1];
        cell.val = value;
      }
      else {// 3,3情况
        var value = this.board.grid[pos_x][pos_y - 1];
    ...
    else if(!this.board.cellEmpty()) 
    {
      var value = Math.random() < 0.9 ? 2 : 4;// 很大比例都是2
      var cell = this.board.selectCell(); 
      cell.val = value; // 
      this.update(cell);
    }
  },
  update(cell) { // 更新数据 是为了将矩阵中grid数据更新
